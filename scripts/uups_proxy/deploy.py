...
from brownie import TransparentUpgradeableProxy,ProxyAdmin, Contract, accounts, network, config,SimpleContractUUPS, SimpleContractUUPSV2, ProxyContract
import time

class UUPSProxyDeployer:
    ...
    def deploy_proxy_contract(self):
        ...
        self.proxy_contract_object = ProxyContract.deploy(
    ...
    def upgrade_proxy_contract(self,new_implementation_code, new_implementation_address,contract_name):
        ...
        self.implementation_contract_name = contract_name
        # get contract from abi and call upgradeTo function
        proxy_from_abi = Contract.from_abi(contract_name, self.proxy_contract_object.address, self.implementation_contract_object.abi)
        proxy_from_abi.upgradeTo(new_implementation_address, {'from': accounts[0]})
        time.sleep(1)
        return proxy_from_abi
...
def main():
    deployer_object = UUPSProxyDeployer(SimpleContractUUPS, 123)
    ...
    # DEPLOY SimpleContractUUPSV2
    simple_contract_v2 = SimpleContractUUPSV2.deploy({'from': accounts[0]})
    time.sleep(1)

    # UPGRADE PROXY CONTRACT
    deployer_object.upgrade_proxy_contract(SimpleContractUUPSV2,simple_contract_v2.address,"SimpleContractUUPSV2")
    time.sleep(1)
